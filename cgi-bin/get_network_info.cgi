...
if [ -s /config/network.conf ] ; then	
	ant_conf_hostname=`cat /config/network.conf | grep hostname | cut -d'|' -f1`
	ant_conf_hostname=${ant_conf_hostname/hostname=/}
	ant_conf_ipaddress=`cat /config/network.conf | sed 's/[[:space:]]/\n/g' | grep ipaddress`
	ant_conf_ipaddress=${ant_conf_ipaddress/ipaddress=/}
	ant_conf_netmask=`cat /config/network.conf | sed 's/[[:space:]]/\n/g' | grep netmask`
	ant_conf_netmask=${ant_conf_netmask/netmask=/}
	ant_conf_gateway=`cat /config/network.conf | sed 's/[[:space:]]/\n/g' | grep gateway`
	ant_conf_gateway=${ant_conf_gateway/gateway=/}
	ant_conf_dnsservers=`cat /config/network.conf | sed 's/[[:space:]]/\n/g' | grep dnsservers`
