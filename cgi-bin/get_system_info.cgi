...
if [ -s /config/network.conf ] ; then
	...
	ant_host_name=`cat /config/network.conf | sed 's/[[:space:]]/\n/g' | grep hostname | cut -d'|' -f1`
	ant_host_name=${ant_host_name/hostname=/}
	ant_ipaddress=`cat /config/network.conf | sed 's/[[:space:]]/\n/g' | grep ipaddress`
	ant_ipaddress=${ant_ipaddress/ipaddress=/}
	ant_netmask=`cat /config/network.conf | sed 's/[[:space:]]/\n/g' | grep netmask`
	ant_netmask=${ant_netmask/netmask=/}
	ant_gateway=`cat /config/network.conf | sed 's/[[:space:]]/\n/g' | grep gateway`
	ant_gateway=${ant_gateway/gateway=/}
	ant_dnsservers=`cat /config/network.conf | sed 's/[[:space:]]/\n/g' | grep dnsservers`
...
ant_bmminer_version=`bmminer-api -o version`
if [ "${ant_bmminer_version}" == "Socket connect failed: Connection refused" ]; then
	ant_bmminer_version=
else
	ant_bmminer_version=${ant_bmminer_version#*CGMiner=}
	ant_bmminer_version=${ant_bmminer_version%%,API=*}
fi

ant_hwv=`bmminer-api -o stats`
...
echo \"bmminer_version\":\"${ant_bmminer_version}\"
